 ...
 *   - completeMint({...})     -> finish an interrupted bridge from a burn tx hash
...
export async function runBridge({
  ...
  let burnHash = null; // set ONLY after the burn is confirmed on-chain
  ...
  try {
    ...
    const submittedBurnHash = await sourceWallet.writeContract({
    ...
    onStatus({ step: 'burn', status: 'pending', hash: submittedBurnHash, message: 'Waiting for burn to confirm…' });
    await confirm(sourceChain, submittedBurnHash);
    burnHash = submittedBurnHash; // only now is the burn real
    onStatus({ step: 'burn', status: 'done', hash: submittedBurnHash });
    ...
    if (burnHash) {
      // The burn is confirmed, so make it clear the funds are not lost.
      throw new Error(
        `${base} — your USDC was already burned on ${sourceChain.name} (burn tx ${burnHash}). ` +
          `Use "Finish an interrupted bridge" on the bridge page with this burn tx hash to complete the mint on ${destinationChain.name}.`,
        { cause: err }
      );
    }
...
/**
 * Finish a bridge whose burn already succeeded but whose mint was cancelled or failed.
 * @param {object}   params
 * @param {object}   params.sourceChain       chain the USDC was burned on
 * @param {object}   params.destinationChain  chain to mint on
 * @param {string}   params.account           connected wallet address
 * @param {string}   params.burnHash          burn transaction hash on the source chain
 * @param {string}   [params.networkMode]     'testnet' | 'mainnet'
 * @param {function} [params.onStatus]        ({ step, status, hash?, message? }) => void
 * @returns {Promise<{ mintHash: string }>}
 */
export async function completeMint({
  ...
  if (!sourceChain || !destinationChain) throw new Error('Missing source or destination chain.');
  if (!account) throw new Error('Wallet not connected.');
  if (!burnHash) throw new Error('Enter the burn transaction hash.');
  if (sourceChain.requiresCredentials || destinationChain.requiresCredentials) {
    throw new Error('One of the selected chains is not available yet.');
  }
  ...
  try {
    onStatus({ step: 'attestation', status: 'pending', message: 'Fetching Circle attestation…' });
    const { message, attestation } = await waitForAttestation(irisBase, sourceChain.cctpDomain, burnHash);
    onStatus({ step: 'attestation', status: 'done' });
    ...
    throw new Error(errorText(err), { cause: err });
  }
}