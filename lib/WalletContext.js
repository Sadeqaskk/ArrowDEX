...
export function WalletProvider({ children }) {
  ...
  // NEW: this was the missing piece — executeAgentAction (and anything else
  // that needs to call .writeContract()) needs an actual viem WalletClient,
  // not the raw EIP-1193 provider. Both injected wallets and WalletConnect
  // expose an EIP-1193-compatible `request()` method, so viem's `custom()`
  // transport wraps either one the same way. Memoized so a new client isn't
  // built on every render — only when the provider or account actually changes.
