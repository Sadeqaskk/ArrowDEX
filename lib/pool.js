...
import { POOL_CONFIG, EURC_POOL_CONFIG, ERC20_ABI, ARROW_POOL_ABI } from './poolConfig';
...
// ─────────────────────────────────────────────────────────────────────────
// WUSDC/EURC pool — everything below is new, added alongside the existing
// WUSDC/ARROW functions above without changing any of them. EURC uses 6
// decimals (see EURC_POOL_CONFIG), unlike WUSDC/ARROW which are both 18 —
// every amount below is parsed/formatted against each token's own decimals
// instead of assuming 18 like the functions above do.
// ─────────────────────────────────────────────────────────────────────────

async function approveIfNeededEurcPool(tokenAddress, account, amount) {
  const publicClient = getPublicClient();
  const currentAllowance = await publicClient.readContract({
    address: tokenAddress, abi: ERC20_ABI, functionName: 'allowance',
    args: [account, EURC_POOL_CONFIG.pool.address],
  });
  ...
  const hash = await walletClient.writeContract({
    account, chain: arcViemChain, address: tokenAddress, abi: ERC20_ABI, functionName: 'approve',
    args: [EURC_POOL_CONFIG.pool.address, amount],
  });
  await getPublicClient().waitForTransactionReceipt({ hash });
  return hash;
}

/** Real read for the WUSDC/EURC pool — reserves, LP supply, and the user's LP + token balances. */
export async function getEurcPoolState(userAddress) {
  const client = getPublicClient();
  const hasUser = !!userAddress;
  const account = hasUser ? userAddress : '0x0000000000000000000000000000000000000000';
  ...
  const results = await client.multicall({
    multicallAddress: MULTICALL3_ADDRESS,
    contracts: [
      { address: EURC_POOL_CONFIG.pool.address, abi: ARROW_POOL_ABI, functionName: 'getReserves' },
      { address: EURC_POOL_CONFIG.pool.address, abi: ARROW_POOL_ABI, functionName: 'totalSupply' },
      { address: EURC_POOL_CONFIG.pool.address, abi: ARROW_POOL_ABI, functionName: 'balanceOf', args: [account] },
      { address: EURC_POOL_CONFIG.wusdc.address, abi: ERC20_ABI, functionName: 'balanceOf', args: [account] },
      { address: EURC_POOL_CONFIG.eurc.address, abi: ERC20_ABI, functionName: 'balanceOf', args: [account] },
    ],
  });

  const [reservesR, totalSupplyR, lpBalanceR, wusdcBalanceR, eurcBalanceR] = results;

  if (reservesR.status !== 'success') {
    throw new Error(reservesR.error?.message || 'Failed to read WUSDC/EURC pool reserves.');
  }
  const reserves = reservesR.result;
  const totalSupply = totalSupplyR.status === 'success' ? totalSupplyR.result : 0n;
  const lpBalance = hasUser && lpBalanceR.status === 'success' ? lpBalanceR.result : 0n;
  const wusdcBalance = hasUser && wusdcBalanceR.status === 'success' ? wusdcBalanceR.result : 0n;
  const eurcBalance = hasUser && eurcBalanceR.status === 'success' ? eurcBalanceR.result : 0n;

  return {
    reserveWusdc: formatUnits(reserves[0], EURC_POOL_CONFIG.wusdc.decimals),
    reserveEurc: formatUnits(reserves[1], EURC_POOL_CONFIG.eurc.decimals),
    reserveWusdcRaw: reserves[0],
    reserveEurcRaw: reserves[1],
    totalSupply: formatUnits(totalSupply, 18), // LP token itself is 18 decimals
    totalSupplyRaw: totalSupply,
    lpBalance: formatUnits(lpBalance, 18),
    lpBalanceRaw: lpBalance,
    wusdcBalance: formatUnits(wusdcBalance, EURC_POOL_CONFIG.wusdc.decimals),
    eurcBalance: formatUnits(eurcBalance, EURC_POOL_CONFIG.eurc.decimals),
  };
}

/** Real addLiquidity for WUSDC/EURC — approves both tokens using their real decimals, then deposits. */
export async function addEurcLiquidity({ account, amountWusdc, amountEurc, slippagePct = 1, onStatus }) {
  await ensureArcNetwork();
  const walletClient = getWalletClient();

  const amountA = parseUnits(amountWusdc, EURC_POOL_CONFIG.wusdc.decimals);
  const amountB = parseUnits(amountEurc, EURC_POOL_CONFIG.eurc.decimals);
  const amountAMin = (amountA * BigInt(Math.round((100 - slippagePct) * 100))) / 10000n;
  const amountBMin = (amountB * BigInt(Math.round((100 - slippagePct) * 100))) / 10000n;

  onStatus?.('Checking WUSDC approval…');
  await approveIfNeededEurcPool(EURC_POOL_CONFIG.wusdc.address, account, amountA);

  onStatus?.('Checking EURC approval…');
  await approveIfNeededEurcPool(EURC_POOL_CONFIG.eurc.address, account, amountB);

  onStatus?.('Adding liquidity…');
  const hash = await walletClient.writeContract({
    account, chain: arcViemChain, address: EURC_POOL_CONFIG.pool.address, abi: ARROW_POOL_ABI, functionName: 'addLiquidity',
    args: [amountA, amountB, amountAMin, amountBMin],
  });
...
/** Real removeLiquidity for WUSDC/EURC. LP token itself is always 18 decimals. */
export async function removeEurcLiquidity({ account, lpAmount, onStatus }) {
  await ensureArcNetwork();
  const walletClient = getWalletClient();
  const liquidity = parseUnits(lpAmount, 18);
  ...
  onStatus?.('Removing liquidity…');
  const hash = await walletClient.writeContract({
    account, chain: arcViemChain, address: EURC_POOL_CONFIG.pool.address, abi: ARROW_POOL_ABI, functionName: 'removeLiquidity',
